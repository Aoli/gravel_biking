...
class NvdbService {
  ...
  Future<Map<String, dynamic>> _queryRoadSurfaces(LatLngBounds bbox) async {
    ...
    final uri = Uri.parse('$_baseUrl/api/v2/objekt/97').replace(
    ...
    try {
      ...
      for (final prop in egenskaper) {
        if (prop is Map<String, dynamic>) {
          ...
          if (typeId == 4002 && value != null) { // Vagytans material
  ...
  bool _isGravelSurface(String surfaceType) {
    ...
    return gravelSurfaces.any((surface) => 
      surfaceType.toLowerCase().contains(surface));
  }
  ...
  String _extractRoadClass(Map<String, dynamic> obj) {
    ...
    for (final prop in egenskaper) {
      if (prop is Map<String, dynamic>) {
        ...
        // Road class property
        if (typeId == 1002 && value != null) { // Vaghallare
          return value;
        }
      }
    }
